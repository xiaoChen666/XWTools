...
+(void)hs_updateWithblock:(UpdateBlock)block {
    [HSUpdateApp hs_updateWithAPPID:nil withBundleId:nil block:block];
}
...
+(void)hs_updateWithAPPID:(NSString *)appId withBundleId:(NSString *)bundelId block:(UpdateBlock)block{
    ...
    if (appId != nil) {
        ...
        CXWLog(@"【1】当前为APPID检测，您设置的APPID为:%@  当前版本号为:%@",appId,currentVersion);
        ...
        CXWLog(@"【1】当前为BundelId检测，您设置的bundelId为:%@  当前版本号为:%@",bundelId,currentVersion);
    }else{
        ...
        CXWLog(@"【1】当前为自动检测检测,  当前版本号为:%@",currentVersion);
    }
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if([currentVersion floatValue] < [appStoreVersion floatValue])
            {
                ...
                CXWLog(@"【4】判断结果：当前版本号%@ > 商店版本号%@ 不需要更新\n========我是分割线========",[[NSBundle mainBundle] infoDictionary][@"CFBundleShortVersionString"],appInfoDic[@"results"][0][@"version"]);
