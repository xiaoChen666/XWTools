...
- (void)cyl_checkEmpty {
    ...
    if (!isEmpty != !self.placeHolderView) {
        if (isEmpty) {
            ...
            if (self.tableHeaderView.frame.size.height>KAppHeight-CURRENTSCREN_NAVBAR_HEIGHT) {
                self.placeHolderView.frame = CGRectMake(0, 0, self.frame.size.width, 0);
...
//            CXWLog(@"----%f",self.tableHeaderView.frame.size.height);
            self.placeHolderView.frame = CGRectMake(0, self.tableHeaderView.frame.size.height, self.frame.size.width, self.frame.size.height-self.tableHeaderView.frame.size.height);
            [self addSubview:self.placeHolderView];
        }
    }
}
- (void)cxw_layoutPlaceView {
    [self.placeHolderView removeFromSuperview];
    if (self.tableHeaderView.frame.size.height>KAppHeight-CURRENTSCREN_NAVBAR_HEIGHT) {
        self.placeHolderView.frame = CGRectMake(0, 0, self.frame.size.width, 0);
        [self.placeHolderView removeFromSuperview];
