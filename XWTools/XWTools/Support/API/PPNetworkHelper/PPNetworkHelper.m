...
+ (NSURLSessionTask *)GET:(NSString *)URL
    ...
    if (![[CXWDataManager shareManager] CXWIsNetwork]) {
        [[CXWAlertTools shareTools] CXWAlertErrorMessageWithTitle:@"请先开启网络"];
        failure(nil);
        return nil ;
    }
...
+ (NSURLSessionTask *)POST:(NSString *)URL
    ...
    NSDictionary *newDic = [[CXWDataManager shareManager] CXWBackPublicParameterWithSourceDic:parameters];
    [self CXWSetPulicHead];
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:newDic progress:^(NSProgress * _Nonnull uploadProgress) {
...
//        NSString *downloadDir = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:fileDir ? fileDir : @"Download"];
        NSString *downloadDir = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"Download"];
        if (kStringIsEmpty(fileDir)) {
            NSString *downloadDir = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"Other"];
        }
        //打开文件管理器
        ...
        if (kStringIsEmpty(fileDir)) {
            filePath = [downloadDir stringByAppendingPathComponent:response.suggestedFilename];
        } else {
           filePath = [downloadDir stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@",fileDir,[response.suggestedFilename pathExtension]]];
        }
        //返回文件位置的URL路径
 ...
 */
+ (void)initialize {
    _sessionManager = [[AFHTTPSessionManager manager] initWithBaseURL:[NSURL URLWithString:URL_BASE]];
    _sessionManager.requestSerializer.timeoutInterval = 30.f;
    _sessionManager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/html", @"text/json", @"text/plain", @"text/javascript", @"text/xml", @"image/*", nil];
//    NSString *cerPath = [[NSBundle mainBundle] pathForResource:@"network" ofType:@"cer"];//证书的路径 xx.cer  network certificate smartCard
    NSString *cerPath = [[NSBundle mainBundle] pathForResource:@"network" ofType:@"cer"];
    
    [self setSecurityPolicyWithCerPath:cerPath validatesDomainName:YES];
...
+ (void)CXWSetPulicHead {
    
    [PPNetworkHelper setValue:nil forHTTPHeaderField:@"Authorization"];
    
    if (!kStringIsEmpty([XWUserManager shareManager].userModel.AccessToken)) {
        NSString *tokenvalue = [NSString stringWithFormat:@"BasicAuth %@",[XWUserManager shareManager].userModel.AccessToken];
        [PPNetworkHelper setValue:tokenvalue forHTTPHeaderField:@"Authorization"];
    }  else {
        [PPNetworkHelper setValue:nil forHTTPHeaderField:@"Authorization"];
    }
}
